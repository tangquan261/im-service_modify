...
func GetOnlineStatus(w http.ResponseWriter, req *http.Request) {
	...
	online := model.IsUserOnline(appid, uid)
...
func GetOnlineClients(w http.ResponseWriter, req *http.Request) {
	...
	r := make(map[int64]IMRmodel.IntSet)
	for c := range clients {
		app_users := c.GetAppRoute().GetUsers()
		for appid, users := range app_users {
			if _, ok := r[appid]; !ok {
				r[appid] = IMRmodel.NewIntSet()
			}
			uids := r[appid]
			for uid := range users {
				uids.Add(uid)

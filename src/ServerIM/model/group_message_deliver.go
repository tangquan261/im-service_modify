...
func (storage *GroupMessageDeliver) saveMessage(msg *model.Message) int64 {
	...
	body := msg.ToData()
	var msg_len int32 = model.MSG_HEADER_SIZE + int32(len(body))
	binary.Write(buffer, binary.BigEndian, msg_len)

	model.WriteHeader(int32(len(body)), int32(msg.Seq), byte(msg.Cmd),
	...
	log.Info("save message:", model.Command(msg.Cmd), " ", msgid)
	return msgid
...
//device_ID 发送者的设备ID
func (storage *GroupMessageDeliver) sendMessage(appid int64, uid int64, sender int64, device_ID int64, msg *model.Message) bool {

	PushMessage(appid, uid, msg)

	route := App_route.FindRoute(appid).(*Route)
	if route == nil {
		log.Warningf("can't send message, appid:%d uid:%d cmd:%s", appid, uid, model.Command(msg.Cmd))
		return false
	}
...
func (storage *GroupMessageDeliver) sendGroupMessage(gm *PendingGroupMessage) bool {
	msg := &model.IMMessage{Sender: gm.sender, Receiver: gm.gid, Timestamp: gm.timestamp, Content: gm.content}
	m := &model.Message{Cmd: model.MSG_GROUP_IM, Version: model.DEFAULT_VERSION, Body: msg}
	...
	for _, member := range members {
		...
		notify := &model.Message{Cmd: model.MSG_SYNC_NOTIFY, Body: &model.SyncKey{Sync_key: msgid}}
